...
describe('User Controller routes', () => {
  ...
  it('it should signin a registered guide', async () => {
    return request
      .post(`${baseUrl}/auth/signup`)
      .set('Content-Type', 'application/json')
      .send(mock.validInput1)
  ...
  it('it should signin a registered user', async () => {
    return request
      .post(`${baseUrl}/auth/signup`)
      .set('Content-Type', 'application/json')
      .send(mock.validInput2)
  ...
  it('it should register and signin another guide ', async () => {
    return request
      .post(`${baseUrl}/auth/signup`)
      .set('Content-Type', 'application/json')
      ...
      .then(async () => {
        const res = await request
          .post(`${baseUrl}/auth/signin`)
          .set('Content-Type', 'application/json')
          .send(mock.guideSignin);
        user3Token.data = res.body.body.token;
        expect(res.statusCode).toBe(200);
        expect(res.body.success).toEqual(true);
        expect(res.body.body.token);
        expect(res.body.body.email).toEqual('frodo@yahoo.com');
        expect(res.body.body.userName).toEqual('Baggins');
        expect(res.body.body.guide).toEqual(true);
      });
  });
  it('should not let unregistered users signin', async () => {
    const res = await request
      .post(`${baseUrl}/auth/signin`)
      .set('Content-Type', 'application/json')
